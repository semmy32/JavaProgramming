...
public class IteratingMulDimArr2 {
    public static void main(String[] args) {
        ...
        for (int i = arr2D.length - 1; i >= 0; i--) {  // i: indexes of each 1D array started from last index

            for (int j = arr2D[i].length - 1; j >= 0; j--) { // j: indexes of each element starting from last index
                System.out.print( arr2D[i][j]+" " ); //not println! attention
            }

            System.out.println();  //we write this for print statement our new array to another line
